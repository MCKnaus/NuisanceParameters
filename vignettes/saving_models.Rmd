...
The sample is drawn from the 1991 Survey of Income and Program Participation (SIPP) and consists of 9,915 observations. To speed up the running time, we will use a 25% subset of the data.
...
We will specify a generic list of base learners for the ensemble. You can find the full list of available learners and their default settings by typing `?create_method` in the console. For more details on the different types of stacking and hyperparameter tuning, see the *Stacking* and *Tuning* notebooks.
...
In this example, we use 3-fold cross-fitting (2/3 of the observations are used to train the machine learner in each cross-fitting split).
...
Models can be saved to a user-specified path by selecting `storeModels = "Disk"`.
...
np <- nuisance_parameters(NuPa = c("Y.hat","Y.hat.d","D.hat","Z.hat"),
                          X = X, Y = Y, D = D, Z = Z,
                          methods = methods, cf = 3, stacking = "short",
                          storeModels = "Disk", path = tmpdir)
```

Later, smoother matrices can be extracted simply by providing the path to `get_outcome_weights`:

```{r}
S <- get_outcome_weights(
  object = file.path(tmpdir, "nuisance_models.rds"), 
  NuPa = "Y.hat"
)
...
```{r}
np <- nuisance_parameters(NuPa = c("Y.hat","Y.hat.d","D.hat","Z.hat"),
                          X = X, Y = Y, D = D, Z = Z, stacking = "short",
                          methods = methods, cf = 3,
                          storeModels = "No")
str(np, 1)
```
