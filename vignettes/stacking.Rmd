...
The estimation routine of `nuisance_parameters` automatically checks the defined methods and excludes those that are not suitable for a given nuisance parameter (some methods are excluded as smoother extraction is not implemented). For example, logit will not be applied when estimating the outcome nuisance parameter, $E[Y \mid X]$.
...
There are several ways to combine base learner predictions into the final ensemble estimate:
