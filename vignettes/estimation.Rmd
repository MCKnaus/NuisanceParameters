...
The package provides a wide range of base learners. You can see a detailed description of the available learners and their default settings by typing `?create_method` in the console.

There are two main ways to register a list of methods. The idea is simple: pick a base learner from the available options, add any optional arguments (like `num_trees` for GRF’s forest), and, possibly, select a subset of covariates to use for estimation. For example, tree-based methods typically exclude the interaction terms used by Lasso.
...
The estimation routine of `nuisance_parameters` automatically checks the defined methods and excludes those that are not suitable for a given nuisance parameter. For example, a logit will not be applied when estimating the outcome nuisance parameter, $E[Y \mid X = x]$.
...
Learners can be combined into an ensemble through stacking. Following [Ahrens et al. (2024)](https://arxiv.org/abs/2401.01645), we implement two variants: standard stacking and short stacking.

#### Standard stacking

In standard stacking, each learner is trained and validated within the folds of the cross-fitting procedure. A second layer of cross-validation is used to determine ensemble weights, which are estimated with non-negative least squares. This method is statistically solid but can be very computationally expensive: with $F$ folds, $V$ validation splits, and $M$ learners, complexity grows roughly with $F \times V \times M$.
...
-   For the regression forest, `grf`'s default tuning routine is run. If `arguments = "tune_on_fold"`, tuning happens on the estimation part of the cross-fitting split (using $F–1$ folds), which is roughly $F$ times more computationally intensive.

-   XGBoost is tuned via the Hyperband-style multi-armed bandit approach: it progressively allocates resources (boosting rounds) to the most promising configurations across successive evaluation rungs.
...
The sample contains 9,915 observations. We use a subset to speed up computation:
