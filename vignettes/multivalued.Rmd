...
```{r}
library(conflicted)
invisible(lapply(c("create_method", "design_matrix", "data_screen", "prep_cf_mat"), 
...
The first is stacked non-negative least squares (NNLS): all treatment-specific estimates are stacked across $K$, a single outcome vector is formed, and $X$ is flattened into a $(KN \times M)$ matrix, on which `nnls::nnls` is applied.
...
We define a flexible set of base learners. For propensity score estimation, we explore all available multiclass strategies ("one-vs-one", "one-vs-rest" and "multiclass").
...
methods = list(
 ...
 "ranger" = create_method("ranger", multinomial = "one-vs-one"),
