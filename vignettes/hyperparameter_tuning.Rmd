...
Hyperparameter tuning is supported by grf's regression forest and XGBoost and can be done either on the full sample or "on-the-fold." If `tuning = "fold"`, tuning takes place on the estimation part of the cross-fitting split, which is roughly $F$ (the number of cross-fitting folds) times more computationally intensive than using the full sample.

-   For the regression forest, grf's default tuning routine is used.
...
The sample contains 9,915 observations. We use a 50% subset to speed up computation:
...
cat("Ensemble:\n"); print(get_rmse(np$nuisance_parameters$Y.hat, Y))
cat("\nBase Learners:\n"); print(as.matrix(apply(cf_preds, 2, get_rmse, Y)))
```
