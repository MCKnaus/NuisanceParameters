...
-   XGBoost is tuned via a hyperband-style multi-armed bandit approach: it progressively allocates resources (boosting rounds) to the most promising configurations across successive evaluation rungs (see `?xgboost_tune`).

-   Tuning for ranger is coming form the `tuneRanger` package (find details [here](https://github.com/PhilippPro/tuneRanger?tab=readme-ov-file)).

See [Bach et al. (2024)](https://arxiv.org/abs/2402.04674) for a discussion of hyperparameter tuning in Double Machine Learning.
...
mtd_full = list(
 "ols" = create_method("ols"),
 "ranger_full" = create_method("ranger", tuning = "full_sample"),
...
mtd_fold = list(
 "ols" = create_method("ols"),
 "ranger_full" = create_method("ranger", tuning = "fold"),
