...
```{r}
library(conflicted)
invisible(lapply("get_outcome_weights", function(x) conflict_prefer(x, "NuisanceParameters")))
...
Next, we will specify a list of base learners for the ensemble. The package offers a great variety of choices—you can find the full list of available learners and their default settings by typing `?create_method` in the console. For more details on the different types of stacking and hyperparameter tuning, see the *Stacking* and *Tuning* notebooks.

The logic is simple: choose a base learner from the available methods, add optional arguments (for example, `num.trees` for grf’s forest), and, if desired, specify a subset of covariates to use for estimation.
