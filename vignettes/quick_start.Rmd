...
4.  Target parameter estimation and smoother matrix extraction \
    (will be delegated to other packages in the trilogy)

Find more advanced topics in the *Stacking*, *Hyperparameter Tuning*, and *Multivalued Treatments* vignettes.
...
Use `get_smoother_weights` to extract outcome smoother matrices using the trained models saved in the previous step (will be soon handled by the `OutcomeWeights` package):
