...
np_short <- nuisance_parameters(NuPa=c("Y.hat","Y.hat.d","Y.hat.z","D.hat","D.hat.z","Z.hat"),
                                ...
                                cluster = NULL, stratify = FALSE, learner = c("both"), 
...
Issue: do not understand the difference between S- and T- learners...
...
```{r}
debug(NuisanceParameters::get_outcome_weights)
S <- NuisanceParameters::get_outcome_weights(
  np_object = np_short, 
  NuPa = c("Y.hat.d","Y.hat.z","D.hat","D.hat.z","Z.hat")) # "Y.hat",

# Check if SY = Y_hat
check_pairs <- list(
  #list(A = S$Y.hat_ml %*% Y,         B = np_short$nuisance_parameters$Y.hat),
  list(A = S$Y.hat.d_ml[[1]] %*% Y,  B = np_short$nuisance_parameters$Y.hat.d[, 1]),
  list(A = S$Y.hat.d_ml[[2]] %*% Y,  B = np_short$nuisance_parameters$Y.hat.d[, 2]),
  list(A = S$Y.hat.z_ml[[1]] %*% Y,  B = np_short$nuisance_parameters$Y.hat.z[, 1]),
  list(A = S$Y.hat.z_ml[[2]] %*% Y,  B = np_short$nuisance_parameters$Y.hat.z[, 2]))
...
At this point, the `get_outcome_weights` function is a bit of a misnomer: it gives smoother matrices instead of outcome weights. In the future, it will also take in the target parameters estimated in `MLeffects` or elsewhere—and provide a vector of outcome weights.
