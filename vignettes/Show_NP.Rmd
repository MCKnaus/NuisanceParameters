...
That said, I can do it internally instead—when you just pass `W`, `Z`, and `cf`, and `nuisance_parameters` handles the rest, returning `cf_mat`. This would simplify the input.
...
np_stand <- nuisance_parameters(NuPa = c("Y.hat","Yw.hat","Yz.hat","W.hat", "Wz.hat", "Z.hat"),
                      ...
                      cf_mat = cf_mat, w_mat = w_mat, z_mat = z_mat, 
                      cv = cv, learner = c("t"), 
                      storeModels = "Disk", path = "/Users/romanrakov/Desktop", quiet = FALSE)
})
```
...
As of now, only `grf`'s regression forest can be tuned on full sample by specifying `arguments = list("tune_full")`:
...
Short stacking tuned:       ", time_short_tune[3], "s  
Standard stacking not tuned:", time_stand_notune[3], "s
Standard stacking tuned:    ", time_stand_tune[3], "s"
    )
```
