test_that("check predictions and smoother weights of ensemble_short function", {
  ...
  ens = ensemble_short(method, X = X, Y = Y, cf_mat = cf_mat, storeModels = "Memory")
  pred = predict(ens)
  weights_mat = weights(ens, method = method, X = X, Y = Y, cf_mat = cf_mat)
...
# 
#   ens_mem = ensemble_short(method, X = X, Y = Y, cf_mat = cf_mat, storeModels = "Memory")
#   weights_mat_mem = weights(ens_mem, method = method, X = X, Y = Y, cf_mat = cf_mat)
# 
#   ens_disk = ensemble_short(method, X = X, Y = Y, cf_mat = cf_mat, storeModels = "Disk", path = "..")
#   expect_true(is.character(ens_disk$method))
#   expect_true(file.exists(ens_disk$method))
#   weights_mat_disk = weights(ens_mem, method = method, X = X, Y = Y, cf_mat = cf_mat)
# 
#   ens_no = ensemble_short(method, X = X, Y = Y, cf_mat = cf_mat, storeModels = "No")
#   expect_error(weights(ens_no, method = method, X = X, Y = Y, cf_mat = cf_mat), "Ensemble models were not saved after training.")
# 
#   expect_identical(weights_mat_mem, weights_mat_disk)
# 
#   file.remove(ens_disk$method)
# 
# })
...
# 
#   ens_t = ensemble_short(method, X = X, Y = Y, subset = subset, cf_mat = cf_mat, learner = "t", storeModels = "Memory")
#   nnls_w_t = nnls_weights(ens_t$fit_cv[subset, ], Y[subset])
#   ens_s = ensemble_short(method, X = X, Y = Y, subset = subset, cf_mat = cf_mat, learner = "s", storeModels = "Memory")
#   nnls_w_s = nnls_weights(ens_s$fit_cv[subset, ], Y[subset])
#   ens_both = ensemble_short(method, X = X, Y = Y, subset = subset, cf_mat = cf_mat, learner = "both", storeModels = "Memory")
#   nnls_w_both = nnls_weights(ens_both$fit_cv[subset, ], Y[subset])
# 
#   expect_identical(ncol(ens_t$fit_cv), length(method))
#   expect_identical(ncol(ens_s$fit_cv), length(method))
#   expect_identical(ncol(ens_both$fit_cv), as.integer(length(method)*2))
# 
#   pred_t = predict(ens_t, nnls_w_t)
#   weights_mat_t = weights(ens_t, method = method, X = X, Y = Y, subset = subset, w = nnls_w_t, cf_mat = cf_mat)
#   pred_s_t = as.vector(weights_mat_t %*% Y)
# 
#   pred_s = predict(ens_s, nnls_w_s)
#   weights_mat_s = weights(ens_s, method = method, X = X, Y = Y, subset = subset, w = nnls_w_s, cf_mat = cf_mat)
#   pred_s_s = as.vector(weights_mat_s %*% Y)
# 
#   pred_both = predict(ens_both, nnls_w_both)
#   weights_mat_both = weights(ens_both, method = method, X = X, Y = Y, subset = subset, w = nnls_w_both, cf_mat = cf_mat)
