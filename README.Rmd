...
`NuisanceParameters` lets you estimate conditional expectations that can later be used to estimate target causal parameters of interest. Among others, it is an implementation of double/debiased machine learning framework as proposed by Chernozhukov et al. (2018). 
...
1.  `NuisanceParameters` – flexibly estimates $m(X):=\mathbb{E}[Y \mid X]$, $m_w(w,X):=\mathbb{E}[Y \mid W = w, X]$, $e(X) := \mathbb{P}[W \mid X]$ and other user-specified nuisance parameters.
