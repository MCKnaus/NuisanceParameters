...
logit_nnet_fit <- function(X, Y, arguments = list()) {
  if (!requireNamespace("nnet", quietly = TRUE)) {
    stop(
      "The 'nnet' package is not installed. Multinomial logit is optional (in Suggests).\n",
...
nb_gaussian_fit <- function(X, Y, arguments = list()) {
  if (!requireNamespace("naivebayes", quietly = TRUE)) {
    stop(
      "The 'naivebayes' package is not installed. Gaussian Naive Bayes is optional (in Suggests).\n",
...
nb_bernoulli_fit <- function(X, Y, arguments = list()) {
  if (!requireNamespace("naivebayes", quietly = TRUE)) {
    stop(
      "The 'naivebayes' package is not installed. Bernoulli Naive Bayes is optional (in Suggests).\n",
      "Install it with: install.packages('naivebayes')",
      call. = FALSE
    )
  }
...
xgboost_prop_fit <- function(X, Y, arguments = list(nrounds = 40, eta = 0.01)) { # Maren's settings
  ...
  if (K == 2) {
    model <- do.call(xgboost::xgboost, c(list(
      data = X, label = Y, verbose = 0,
      objective = "binary:logistic", eval_metric = "logloss"
    ), arguments))
  } else {
    if (min(Y) == 1) Y <- Y - 1
    model <- do.call(xgboost::xgboost, c(list(
      data = X, label = Y, num_class = K, verbose = 0,
...
svm_fit <- function(X, Y, arguments = list(gamma = 0.1)) {
  if (!requireNamespace("e1071", quietly = TRUE)) {
    stop(
      "The 'e1071' package is not installed. SVM learner is optional (in Suggests).\n",
  ...
  model <- do.call(e1071::svm, c(list(
    y = Y, x = X, probability = TRUE, kernel = "radial",
...
knn_prop_fit <- function(X, Y, arguments = list(kmax = floor(0.05 * nrow(X)))) {
  if (!requireNamespace("kknn", quietly = TRUE)) {
    stop(
      "The 'kknn' package is not installed. k-Nearest Neighbor is optional (in Suggests).\n",
...
ovo_fit <- function(X, Y, method = "logit", parallel = FALSE, verbose = TRUE) {
  ...
  if (use_parallel) {
    ...
    if (parallel && verbose) {
      message(
        "Parallel backend not available. Falling back to sequential OvO fitting.\n",
...
predict.ovr_fit <- function(ovr_fit, X, Y, Xnew = NULL, weights = FALSE, method = "logit") {
  ...
  for (i in seq_len(n_classifiers)) {
    ...
    if (is.list(fit_raw) && !is.data.frame(fit_raw)) {
      fit_raw <- fit_raw$prediction
    }
