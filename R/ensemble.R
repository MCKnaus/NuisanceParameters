...
#'
#' @param method List of methods built via \code{\link{create_method}} to be used in ensemble model
#' @param X Covariate matrix of training sample.
#' @param Y Vector of outcomes of training sample.
#' @param nfolds Number of folds used in cross-validation of ensemble weights (default: 5)
...
ensemble <- function(method,
  ...
  if (length(method) > 1) {
    for (i in 1:ncol(cv_mat)) {
      ...
      if (length(dim(fit_cv)) == 3) { fit_cv[fold, , ] <- preds } else {
        fit_cv[fold, ] <- preds }
    }
    ...
    if (is_mult) {
      ...
      X_stack <- sapply(dimnames(fit_cv)[[3]], function(m) as.vector(fit_cv[, , m]))
      nnls_w <- nnls_weights(X = X_stack, Y = Y_stack)
      
      # Or proceed in a usual fashion
    } else { nnls_w <- nnls_weights(X = fit_cv, Y = Y) }

    # Re-run all methods on the full sample
...
predict.ensemble <- function(object,
  ...
  if (length(object$method) > 1) {
    pred <- predict.ensemble_core(object = object$method_fit, method = method, X_tr = X, Y_tr = Y, X_te = Xnew, 
