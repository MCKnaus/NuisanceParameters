...
get_xgboost_weights <- function(model, dtrain, dtest, save_output = FALSE, output_dir = NULL) {
  ...
  errors <- c()
  base_score <- NULL
  if (isTRUE(all.equal(mean(xgboost::getinfo(dtrain, "label")), model_params$base_score))) {
    base_score <- "mean"
  } else if (model_params$base_score == 0) {
    base_score <- 0
  }
  ...
  if (!is.null(model_params$base_score) && is.null(base_score)) {
    errors <- c(errors, "base_score must be 0 or 'mean'")
  }
  ...
  smoothers <- create_S_from_gbtregressor(
    model, leaf_indices_train, leaf_indices_test, base_score = base_score, 
...
reconstruct_predictions <- function(bst, dtest) {
  ...
  booster <- xgboost::xgb.Booster.complete(bst)
  trees <- xgboost::xgb.model.dt.tree(model = booster)
