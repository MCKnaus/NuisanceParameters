...
nuisance_parameters <- function(NuPa = c("Y.hat", "Y.hat.d", "Y.hat.z", "D.hat", "D.hat.z", "Z.hat"),
  ...
  # Process methods
  method <- create_final_method(method = method, NuPa = NuPa, K = K)

  # Create progress bar
  pb <- if (isFALSE(quiet)) {
    setup_pb(NuPa = NuPa, n_d = ncol(d_mat), n_z = ncol(z_mat), cf_folds = cf, cv_folds = cv, models = method)
  } else { NULL }
...
#'   \item \code{ens_weights}: A list of stacking weights
#'  }
#'
#' @keywords internal
#'
nuisance_cf <- function(method, 
                        Y, X, 
                        cf_mat,
                        cv = 5,
                        subset = NULL,
  ...
  if (cv == 1) {
    ...
    if (is_mult) {
      Y_stack <- as.vector(one_hot(Y))
      X_stack <- sapply(dimnames(ens$fit_cv)[[3]], function(m) as.vector(ens$fit_cv[, , m]))
      nnls_w <- nnls_weights(X = X_stack[subset, ], Y = Y_stack[subset])
      np <- apply(ens$fit_cv, c(1, 2), function(x) sum(x * nnls_w))
      
      # Or proceed in a usual fashion
    } else {
    ...
    for (i in 1:ncol(cf_mat)) {
      ...
      ens <- ensemble(
        method = method_tuned, X = X_tr, Y = Y_tr, nfolds = cv,
        quiet = quiet, pb = pb, pb_np = pb_np, pb_cf = i
      )

      if (is_mult) {
        raw_pred <- predict.ensemble_core(object = ens$method_fit, method = method, X_tr = X_tr, Y_tr = Y_tr, X_te = X_te, 
                                          quiet = quiet, pb = pb, pb_cf = i, pb_cv = ".", pb_np = pb_np)
        ens_pred <- apply(raw_pred, c(1, 2), function(x) sum(x * ens$nnls_weights))
        np[fold] <- ens_pred
      } else {
