...
`create_method`. For binary outcomes, binary treatments, and any
instrument, all learners are available. Some learners are not
implemented for certain nuisance parameters and will be filtered out:
restrictions apply for continuous outcomes (column 3) and multivalued
treatments (column 4):
